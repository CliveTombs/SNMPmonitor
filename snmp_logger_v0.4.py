...
def splash():
    '''
    Generated from http://patorjk.com/software/taag/#p=display&f=Big&t=Tryo%20Config%20Grabber using font "Big" and the Character Height "Smush(R)"
...
def input_filename(filelist):
    ...
    print(filelist)
    datafile = input("\nWhat is the name of the zipfile containing \"unitdetails.csv\"? - ")
    password = input("\nPassword Please? - ")
    ...
    try:
        ...
        datafile, password = input_filename()
...
def read(SITE, AMP, READCOMMUNITY, SNMPV, IP, NUMOID, OID):
    ...
    for r in range(NUMOID):
        print("\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b", "OID", r+1, "of",
              NUMOID, end="            ", flush=True)  # lots of backspaces
        try:
            session = SNMP(IP, community=READCOMMUNITY, version=SNMPV, timeout=2, retries=3)
            v = OID[r]
            OIDValue.append(str(session.get(v)))
